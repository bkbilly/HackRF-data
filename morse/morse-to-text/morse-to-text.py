...
import csv
# from numpy.np.fft import np.fft
...
#         pyplot.plot(data)
#         if length != -1:
#             pyplot.axis(xmax=length)
#         if height != -1:
#             pyplot.axis(ymax=height)
#         pyplot.savefig(name + "." + self.format, format=self.format, dpi=dpi)
#         pyplot.cla()

#     def specgram(self, name, signal):
#         spectrogram = pyplot.specgram(signal)
#         pyplot.savefig(name + "." + self.format, format=self.format)
#         pyplot.cla()
#         return spectrogram
...
class DummyPlotter:
    ...
    def specgram(self, name, signal):
        spectrogram = pyplot.specgram(signal, Fs=2)
        pyplot.cla()
        return spectrogram
...
class SoundFile:
    ...
    def __init__(self, path):
        ...
        while pow(2, power) < self.length:
            power += 1
        self.data = np.append(self.data, np.zeros(pow(2, power) - self.length))
...
class SignalFilter:
    ...
    def filter(self, soundfile):
        ...
        trans = np.fft.rfft(soundfile.getdata())
        ...
        hzignored = 200
        frec = hzignored + np.argmax(trans_real[hzignored:])
        # print max(trans_real)
        # print trans_real[frec]
        # print frec
        mmin = int(frec - band / 2) if (frec > band / 2) else 0
        filter_array = np.append(np.zeros(mmin), np.ones(band))
        filter_array = np.append(filter_array, np.zeros(
            len(trans_real) - len(filter_array)))
        filtered_array = np.multiply(trans, filter_array)
        # plotter.saveplot("filtered_trans", abs(filtered_array))
        #4 - antitransformo
        filtered_signal = np.array(np.fft.irfft(filtered_array)[
            :soundfile.getlength()], dtype="int16")
...
class SpectreAnalyzer:
    ...
    def sumarizecolumns(self, mat):
        vec_ones = np.ones(len(mat))
        vec_sum = (np.matrix(vec_ones) * np.matrix(mat)).transpose()
        # plotter.saveplot("frecuency_volume", vec_sum)
        return vec_sum

    def findpresence(self, vec_sum):
        presence = np.zeros(len(vec_sum))
...
class ShortLong:
    def __init__(self, shorts, longs):
        self.shortmean = np.NaN
        self.shortstd = np.NaN
        self.longmean = np.NaN
        self.longstd = np.NaN
        if len(shorts) > 0:
            self.shortmean = np.mean(shorts)
            self.shortstd = np.std(shorts)
        if len(longs) > 0:
            self.longmean = np.mean(longs)
            self.longstd = np.std(longs)
...
class PulsesAnalyzer:
    ...
    def split(self, vec):
        onesl = np.zeros(1 + len(vec) // 2)
        zerosl = np.zeros(len(vec) // 2)
...
code_result = str_translator.totext(code_string)
